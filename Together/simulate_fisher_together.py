...
def line_process(leng, signal, variance, contrue, num):
    ...
    out_fisher = "simulation-fisher_together_"+"len"+str(leng)+"sig"+str(signal)+"vari"+str(variance)+"noise"+str(contrue)
    out_proposed = "simulationout-proposedmcmcp_together_len"+str(leng)+"sig"+str(signal)+"vari"+str(variance)+"noise"+str(contrue)
    out_existing = "simulationout-proposedmcmcn_together_len"+str(leng)+"sig"+str(signal)+"vari"+str(variance)+"noise"+str(contrue)
    ...
    for i in range(5):
        ...
        cmd = "sh throw_child_fisher_together.sh "+out_fisher+" "+out_proposed+" "+out_existing+" "+out_poisson_p+" "+out_poisson_s+" "+str(i)+" "+str(leng)+" "+str(signal)+" "+str(variance)+" "+str(contrue)+" "+str(num)+" "+str(i)+""
        os.system(cmd)
    """
    tmp = 1
    while tmp > 0:
